...
# DECISION REGISTER — ARCHIVE, D1 .. D169

Split out of `DECISIONS.md` on 2026-08-08 for one reason only: the combined file
reached **1.08 MB** and GitHub does not render markdown above ~1 MB, so the
register had become unreadable in a browser — the exact opposite of what a
public register is for.

**Nothing was edited, reordered, or removed.** This is a byte-exact split: the
two files concatenated reproduce the original. Current entries (D170 onward,
including everything from the bet-time leak discovery through the present) live
in `DECISIONS.md`.
...
- D67 IMPLEMENTATION-GAP REVIEW (3 probe-agents + external review 5): Sean's thesis
...
- D77 MARKET-BLOWUP AUTOPSY (150 worst L_mkt games + external review): market blowups are
...
- D79 PROPS BUG CLUSTER (external review source review): ALL REAL, FIXED in props.py —
...
- D81 LIVE-PATH FIX CLUSTER: the external review’s "tank dead live" REFUTED (stale bundle;
...
- D87 ASSIST-EXPOSURE MINUTES-COUPLING (external review 6 props item) FAILS decisively —
...
- D89 EXTERNAL REVIEW 7 REGIME RECONCILIATION + LOOSE-ENDS SWEEP (thread-6): the external review’s
  ...
  the external review’s role/system state (its #1 bet, already banked); attractor split ==
...
- D95 PAID RETROSPECTIVE (Sean: "figure out what we could have paid for"):
  ...
  external review money-spec into a final worth-it table with measured/bounded values:
...
- D125 BET-ENGINE 3-VIEW RESTRUCTURE SHIPPED (the external review product pass
...
- D127 DEFENCE-CONDITIONED POSSESSION MARGIN — LANE KILLED at the
  pre-registered dev gate (the #1-ranked open lane from the external review product
...
- D128 D79 REVERSAL REVIEW SETTLED — one KEEP, one REVERT, on the endpoint the
  ...
  (A) 002-FILTER ON THE PROPS EWMA -> **KEEP THE FIX** (do not revert).
    ...
    early-minutes-ramp prize (external review product pass item B2), now quantified.
  (B) KALMAN FORWARD STEP -> **REVERTED** (nbapred/engine/props.py). Primary
    ...
    NOT LIVE / no named future gate, per the external review product-pass rule that
...
- D130 LATE-SEASON URGENCY — ALL THREE PRE-REGISTERED ARMS FAIL THE DEV GATE.
  The last surviving sides re-test from the external review product pass (its B3) and the
...
- D134 REGISTER + ARTIFACT HYGIENE (no renumbering — aliases only, per the
  external review product-pass rule "if cleanup breaks old citations, do not do it").
