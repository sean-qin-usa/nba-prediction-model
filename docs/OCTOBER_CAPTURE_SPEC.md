...
**A DEFECT IN THIS SECTION AS FIRST WRITTEN, FOUND 2026-08-08.** The plan below
assumes the report poller produces TRANSITIONS. It cannot today, and not for a
software reason: **the NBA publishes several injury-report editions per day, and
this project has only ever downloaded the 5PM one.** `injury_reports_pit.edition`
has cardinality **1** across all 125,704 rows, `report_date` is a `DATE` not a
timestamp, and all **1,260 raw PDFs on disk are 5PM editions**. So every "event"
would carry the same clock time each day, the 30-minute pairing window in §3
would be comparing one edition against itself, and §5 would return a confident
null that means nothing.

**PREREQUISITE, ahead of everything else in this document: the poller must fetch
EVERY published edition, not just 5PM, and stamp each with `observed_ts` at
retrieval.** Without intra-day editions there are no injury events to pair prices
against, and the entire injury-response channel is unmeasurable no matter how
good the price capture is. This is data acquisition, not modelling, and it is
cheap — the same PDF endpoint with a different edition key.
...
## 5b. WHICH CLOCKS EXIST TODAY

Answering "can we use the timestamps?" with the inventory rather than intent:

| clock | status | note |
|---|---|---|
| our receipt time (`snapshot_ts`) | **WORKS** | written per poll, fsync'd |
| book post time (`book_last_update`) | **WORKS** | in `odds_quotes`, populated by `load_odds.py`; this is what separates a real quote update from a stale book re-polled |
| injury event time | **MISSING** | see the defect note in §2 — 5PM editions only |
| historical anything | **ABSENT** | `odds_quotes` 0 rows; the whole historical odds stack is one row per game with a two-value open/close `phase` and NO timestamp column |

Two of the three prospective clocks already work. The third is a download
change. Nothing historical is recoverable: there is no timestamp PAIR anywhere
in held data that could measure a market response to an injury event, which is
why that channel is a 2027 result and not a 2026 one.
