...
# IN SEASON nothing should be quiet this long. OFFSEASON the logger idles at
# ODDS_IDLE_SLEEP_MIN (12h by default), so a fixed 8h threshold fails EVERY DAY
# of the offseason -- which is exactly how a monitor gets muted before the one
# night it matters. The threshold follows the mode the logger is actually in.
STALE_MIN_INSEASON = 8 * 60
IDLE_SLEEP_MIN = float(os.environ.get("ODDS_IDLE_SLEEP_MIN", 720))
STALE_MIN_OFFSEASON = IDLE_SLEEP_MIN * 1.5
...
def main() -> int:
    ...
    n_ev_last = max((len(r["data"]) for r in recs if r.get("kind") == "main"
                     and isinstance(r.get("data"), list)), default=0)
    idle = n_ev_last == 0
    thresh = STALE_MIN_OFFSEASON if idle else STALE_MIN_INSEASON
    chk("capture is fresh", age <= thresh,
        f"last snapshot {age:.0f}m ago (threshold {thresh:.0f}m, "
        f"{'idle/offseason' if idle else 'in-season'} cadence)")
