#!/usr/bin/env python3
"""BET-SIM 2 — rerun of the strict IS/OOS betting simulator on the NEW
tank-shipped predictions (D73). Copy of scripts/bet_sim.py, repointed.

DATA: data/capstone_pergame_tank.csv (p_us = our model home win prob WITH the
production April-tank term, p_mkt = DE-VIG CLOSE derived from the spread;
NEW columns vs carry2: tsd = tank-score diff (home-away, nonzero only inside
the tank window, i.e. either team gp >= 55 — verified to coincide exactly,
1242/3690 rows), k = walk-forward tank coefficient) + data/nba.duckdb
(read_only) for games-played (W6) and star-out flags (game_inactives join).

PRICING ASSUMPTIONS (identical to bet_sim.py; stated up front, no line
shopping, always get the close):
  ...
  * ROI is also reported at the FAIR (no-vig) close, decimal = 1/p_mkt_side,
    ...
    printed for EVERY rule, IS and OOS.

PRE-REGISTERED RULE FAMILY (ALL 15 evaluated in-sample; no post-hoc adds):
  ...
  R3(t): R1(t) AND star-out game (either team has an inactive with trailing
         ...
         the game — scripts/ba_intersection.star_out_map, game_inactives
         join) — only where inactives coverage exists (see caveat).
  R4(t): R1(t) AND tank window: either team gp >= 55 (the D73 tank-term
         activation window; replaces bet_sim.py's month-in-{3,4} convention
         by pre-registration — the tank term now prices late-season
         capitulation and improved our predictions ~+0.013/game there)
  R5(t): R1(t) AND |tsd| > 0.5 — games where the NEW tank information is
         largest (tank-score diff column; ~220 games/season)
  Opposite-side games are NEVER bet (known net-negative). Taxonomy anchor
  from bet_sim.py (carry2, full 3 seasons): same-side-we-sharper n=981 hit
  67.6% — new counts differ slightly because p_us changed on the 1242
  tank-window rows.
...
IS/OOS DISCIPLINE (pre-registered selection procedure, mechanical):
  * IN-SAMPLE = 2023-24 + 2024-25. All 15 rules evaluated IS.
  ...
  * OOS = 2025-26, scored ONLY for the selected rules...
  * ...PLUS a pre-committed DIAGNOSTICS section: OOS numbers for all R4/R5
    rules even if not selected, explicitly labeled non-selected — these are
    reported because the tank term is the new information being tested, but
    they carry NO selection protection and must not be treated as validated.
  * HONESTY CAVEAT: the rule FAMILY was motivated by the win-vs-market
    taxonomy, whose aggregate counts were computed on all 3 seasons. OOS is
    clean w.r.t. threshold/family SELECTION (done on IS only), but not
    w.r.t. the discovery that the shared-side-sharper class exists. Treat
    OOS numbers as validation of the selection, not discovery.

RULES HONORED: DuckDB read_only=True; new file scripts/bet_sim2.py only;
...
Run:  python scripts/bet_sim2.py
"""
...
MIN_BETS_IS = 40
N_SELECT = 2
TANK_GP = 55          # D73 tank-term activation window (either team)
TSD_BIG = 0.5         # R5: |tank-score diff| threshold
...
def build_frame() -> pd.DataFrame:
    ...
    df["late"] = (df.h_gp >= TANK_GP) | (df.a_gp >= TANK_GP)   # tank window
    df["tsd_big"] = df.tsd.abs() > TSD_BIG                     # new-info games
    return df
...
def rule_masks(df: pd.DataFrame) -> dict[str, pd.Series]:
    ...
    for t in THRESHOLDS:
        ...
        rules[f"R5(t={t:.2f})"] = base & df.tsd_big
    return rules
...
def gap_to_profit(r) -> str:
    """How much avg price improvement (decimal points on winning bets) would
    zero out a losing rule — the line-shopping/CLV gap."""
    ...
    dd = -r["pnl"] / r["n_wins"]  # extra decimal payout needed per winning 1u
...
def main():
    ...
    # taxonomy anchor cross-check (full 3 seasons; carry2 anchor was 981/67.6%
    # — tank predictions changed p_us on the 1242 tank-window rows, so counts
    # may shift slightly)
    sharper = df.same_side & (df.edge > 0)
    print("=" * 96)
    print("BET-SIM 2 (TANK predictions, D73) — pre-registered rule family, "
          "IS=2023-24+2024-25, OOS=2025-26")
    print("=" * 96)
    print(f"taxonomy anchor: same-side-we-sharper n={int(sharper.sum())} "
          f"hit={df.hit[sharper].mean()*100:.1f}%  (carry2 anchor: 981/67.6%)"
          f"  |  opposite-side n={int((~df.same_side).sum())} NEVER BET")
    print(f"tank window (either gp>={TANK_GP}): "
          f"{int(df.late.sum())} games; |tsd|>{TSD_BIG}: "
          f"{int(df.tsd_big.sum())} games "
          f"({', '.join(f'{s}: {int(n)}' for s, n in df[df.tsd_big].groupby('season').size().items())})")
    ...
    # ---- IN-SAMPLE: all 15 rules x 2 sizings --------------------------------
    print("\n" + "-" * 96)
    ...
    # ---- NON-SELECTED DIAGNOSTICS: R4/R5 OOS (pre-committed in task spec) ---
    print("\n" + "-" * 96)
    print("NON-SELECTED DIAGNOSTICS — R4/R5 OOS numbers (pre-committed "
          "reporting of the tank-information rules; NO selection protection, "
          "NOT validated — informational only)")
    print(HDR)
    for name in rules:
        if not (name.startswith("R4") or name.startswith("R5")):
            continue
        if name in selected:
            print(f"{name:<12}(selected — see OOS table above)")
            continue
        sub = df[rules[name] & oos_m]
        for sz in ("flat", "qkelly"):
            r = score(sub, sz)
    ...
    if verdict_profitable:
        ...
        print("  NO selected rule is OOS-profitable at -110-equivalent "
              "pricing on the tank predictions.")
    ...
    for name, r in oos_gap.items():
        ...
        print(f"    {name:<12}[{tag}] n={r['n']:>3} ROI={r['roi']*100:+7.2f}% "
    ...
    print("  Selection was mechanical on IS flat Sharpe; OOS untouched by "
          "selection. R4/R5 OOS diagnostics were pre-committed but carry no "
          "selection protection. Family-level caveat: the taxonomy motivating "
          "the family was computed on all 3 seasons (see docstring).")
