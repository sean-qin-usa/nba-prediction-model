...
A market-anchored NBA sides system, and the full research record behind it: **a register that runs to D247, most entries rejections.**

> ### 📄 Start here — [**Model and Strategy Review (PDF)**](nba_model_and_strategy_review.pdf)
>
> The main summary document: what the system is, what it returns, and where it
> is weakest, in three pages. Rendered from **[docs/REVIEW.md](docs/REVIEW.md)**
...
The market-blind model does **not** beat the opening line on its own. Its disagreement nevertheless carries information, and shrunk hard against a market anchor it improves on the opener in six of seven scored seasons. That asymmetry — a carefully built fundamental model that loses to the market alone but adds value as a correction to it — is the project's central finding, and it is why the system is built this way.
...
**The model has four inputs. One of them — the daily NBA injury report the entire availability leg is built on — did not exist before 2018-12-17.**
...
1. **Every figure spanning seasons before 2019-20 measures a different, crippled model.** Availability composition is half the production margin; before the feed exists that leg runs on inputs it was never designed to have.
2. **The honest evaluation frame is seven seasons.** Season-clustered intervals on it are tens of ROI points wide, and essentially everything tested is statistically indistinguishable from noise.
3. **The frame is too short to tune on.** A null taking the best of five *randomly chosen* game subsets buys **+2.54 ROI points** (`D187`).
4. **Multi-book execution is largely counterfactual.** The best-of-nine tier is observed in **2023-24 only** (7.74 books/game); 2024-25 and 2025-26 observe 1.00 and 1.03.

Every document in `docs/` carries this caveat in its header. If you quote a number here, quote its frame with it.

---

## The two reporting frames

Each is defined by what data exists, never by which window scored best.
...
**The offset construction improves on the opening line in six of the seven scored seasons** and recovers **24.1%** of the information the market adds between the open and the close. The market-blind model does not improve on the opener and recovers **−36.4%**, moving the wrong way.

Against the closing line the market-blind normalized gap — `(ll_us − ll_mkt) / (ln2 − ll_mkt)`, the share of the closing line's skill-above-a-coinflip left uncaptured — is **13.59%**.

![normalized gap by season](charts/frame_model_2019_26.png)

### The two gates that promoted this to production
...
Both specs were hashed and their minimum detectable effects stated before any endpoint was scored.
...
The gain is model rather than shopping: it holds at a single book.
...
**The link scale 7.2 is a plug-in, not a hand-set constant.** Matching a logistic to the margin-residual normal gives 7.53 from training residuals alone. [docs/CONSTANTS.md](docs/CONSTANTS.md) separates the constants that are derivable from those that are genuinely free.

**Market-blindness is enforced by construction.** `nbapred/market/offset.py` takes the blind margin as an argument and cannot reach back into the model that produced it, so the blind model still never sees a price. With no opening price available the layer returns the blind margin unchanged — a dead odds feed costs the correction, never the prediction.
...
Kept here rather than in the summary, because a results document should not be two-thirds caveat — but none of it is optional reading.
...
**Short windows are selection, not regime.** The best 3-season window returns +16.95% against a **+9.03% average across all 3-season windows**; the best 4-season window +14.92% against +9.33%. Roughly **+5.6 to +7.9 points** of any short-window headline is the act of choosing it (`D208`).

**Beating a permutation null is necessary, not sufficient.** Three selectors each beat their own null (p ≤ 0.048 after correction) and **all three lost to the incumbent** (`D176`).

**The model is era-specific by construction.** Ablating the era-specific context terms flips the sign of the betting result. Zero shipped components can be dated to a gate using only pre-2021 data. **2026-27 is the first genuinely prospective test.**

**CLV is a monitor, not an objective.** A divergence selector bought more CLV and less ROI; a CLV-targeted selector bought no extra CLV and the most ROI (`D176`).
...
- **An availability leak in the certified backtest** — the capstone built injury lists from *tonight's box score*. The live path was clean, so no prediction was ever wrong; the published expectation was too good by 3.8 points of normalized gap (`D158`).
...
- **A chart axis that hid the one season we won** — a hard-coded `ylim(0, …)` clipped 2008-09 off the bottom of the frame (`D171`).

## Charts

`charts/` holds current renders. The ones that carry the argument:
...
Superseded renders are moved to `charts_archive/` in the working tree with a datestamp rather than deleted, so a number that changed can be traced to the run that changed it.
...
Suggested reading order: [docs/REVIEW.md](docs/REVIEW.md) (results), [docs/FAILURES.md](docs/FAILURES.md) (what was tried and why it failed, grouped by cause of failure), [docs/DECISIONS.md](docs/DECISIONS.md) (the register, D170+) and [docs/DECISIONS_ARCHIVE.md](docs/DECISIONS_ARCHIVE.md) (D1-D169), [docs/GATE_POLICY_V2.md](docs/GATE_POLICY_V2.md) (how a change earns its way in), [docs/LEAKAGE.md](docs/LEAKAGE.md), [docs/CONSTANTS.md](docs/CONSTANTS.md), [docs/OCTOBER_RUNBOOK.md](docs/OCTOBER_RUNBOOK.md).
...
Two rules carry most of the weight: **hypotheses come before configurations** (no blind grids), and **a term whose fitted sign contradicts its stated mechanism is a null, however significant it looks.**
...
1. **Capture at least two books at the open, from opening night.** Best-of-two lifts CLV by about 49%, and taking the worse book erases nearly all of it. The logger has never run in-season and `odds_quotes` is empty. **If it is down on opening night, that season's open-price record cannot be reconstructed** — the only genuinely irreversible item on this list.
2. **Run both arms prospectively in 2026-27.** Offset as primary, market-blind as its live control. They share 65% of their bets and take the same side on every overlapping game, so this is a controlled comparison rather than a portfolio (`D205`).
3. **CLV as the live yardstick, not the objective.** Positive in 17 of 19 seasons, in units needing no devig convention, and it resolves in weeks where ROI needs decades.
4. **Exchange access, if it becomes available.** The largest cost lever measured here, and an access problem rather than a modelling one.
5. **Props before sides.** Both shipped improvements of the last cycle came from the props engine.

Explicitly **not** next: further feature search on the sides model. Nineteen seasons, an exhaustion audit, a 49-feature battery and a possession-level rebuild all point the same way, and the capacity number above explains why marginal features keep looking real and failing to transfer.

## Status

The offset construction and soft availability are both in production, both gated. No capital has been deployed. The question the project turns on is whether live CLV against opening prices, with two books captured, reproduces the backtested relationship.
